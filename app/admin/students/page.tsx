  ...
  return (
    <div className="flex flex-col gap-6">
      <section>
        ...
        <p className="mt-2 text-sm text-slate-500">ابحث عن أي طالب، فعّل له كورس يدويًا أو عطّل الحساب عند الحاجة.</p>
      </section>

      <AdminAlert message={success} />
      <AdminAlert message={error} tone="danger" />

      <section className="flex flex-col gap-3 rounded-[24px] border border-white bg-white p-4 shadow-[0_12px_38px_rgba(15,23,42,.05)] sm:flex-row">
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-[28px] border border-white bg-white shadow-[0_12px_38px_rgba(15,23,42,.05)]">
              ...
              <tbody className="divide-y divide-slate-100">
                {students.map((student) => (
                  <tr key={student.id} className="transition hover:bg-slate-50/70">
                    ...
                    <td className="p-4"><span className="inline-flex min-w-10 items-center justify-center rounded-xl bg-blue-50 px-3 py-2 font-mono font-black text-blue-700">{student.subscriptions_count}</span></td>
                    ...
                    <td className="p-4"><div className="flex items-center gap-2"><button onClick={() => openActivation(student)} className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700"><FontAwesomeIcon icon={faUserCheck} /> تفعيل كورس</button><button onClick={() => toggleStatus(student)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-black ${student.status === "active" ? "border-rose-200 bg-rose-50 text-rose-600" : "border-blue-200 bg-blue-50 text-blue-700"}`}><FontAwesomeIcon icon={student.status === "active" ? faBan : faCircleCheck} /> {student.status === "active" ? "تعطيل" : "تفعيل الحساب"}</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <AdminModal open={modalOpen} onClose={() => setModalOpen(false)} title="تفعيل اشتراك يدوي" subtitle={selected ? `سيتم منح ${selected.name} وصولًا مباشرًا إلى الكورس المختار.` : ""} widthClass="max-w-xl">
        <form onSubmit={activate} className="space-y-5">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold leading-6 text-amber-800">التفعيل من هنا يمنح الطالب الوصول، لكنه لا ينشئ إيصال دفع جديد. عمليات فودافون كاش وInstaPay يجب تسجيلها واعتمادها من صفحة المدفوعات للمحاسبة.</div>
